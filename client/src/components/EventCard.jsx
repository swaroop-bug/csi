...
const catColor = { Hackathon: 'var(--color-primary)', Workshop: '#a78bfa', Seminar: '#fbbf24', Competition: '#f87171', Talk: '#4ade80' };
...
export default function EventCard({ ev }) {
  ...
  const handleRegister = () => {
    window.open('https://forms.gle/SLXiBn64Wxtswxja6', '_blank');
  };

  return (
    <>
      <GlassCard style={{ overflow: 'hidden' }}>
        <div style={{ position: 'relative', height: 170, overflow: 'hidden', background: 'var(--color-surface)' }}>
        ...
        <div style={{ padding: '16px 18px' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 5, marginBottom: (ev.hasRegistration && ev.type !== 'past') ? 14 : 0 }}>
            {[{ ic: 'mail', c: 'var(--color-primary)', t: ev.date }, { ic: 'map', c: '#a78bfa', t: ev.loc }, { ic: 'users', c: '#4ade80', t: ev.att ? `${ev.att} attendees` : null }].filter(x => x.t).map(({ ic, c, t }) => (
          ...
          {ev.hasRegistration && ev.type !== 'past' && (
            <button 
              className="btn-pr hero-glow" 
              onClick={handleRegister}
              style={{ width: '100%', padding: '8px', borderRadius: 10, fontSize: '.82rem', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6 }}
            >
              Register Now <Ico name="arrowR" size={12} />
            </button>
          )}
        </div>
      </GlassCard>
