...
const replacements = [
  // Inline RGBs
  { regex: /rgba\(\s*6\s*,\s*182\s*,\s*212/g, replace: 'rgba(124, 58, 237' }, // Cyan -> Primary (Purple)
  { regex: /rgba\(\s*14\s*,\s*165\s*,\s*233/g, replace: 'rgba(167, 139, 250' }, // Secondary cyan -> Accent (Light Purple)
  { regex: /rgba\(\s*5\s*,\s*8\s*,\s*24/g, replace: 'rgba(9, 9, 11' }, // Old Background -> Obsidian
  { regex: /rgba\(\s*15\s*,\s*23\s*,\s*42/g, replace: 'rgba(19, 19, 22' }, // Old Surface -> Surface variant

  // Inline Hex strings with quotes (strict)
  ...
  // Specific inline hex assignments where they might not be enclosed tightly
...
function walk(dir) {
  ...
  files.forEach(file => {
    ...
    if (fs.statSync(fullPath).isDirectory()) {
      ...
      // Don't touch index.css with script, we will overwrite it completely
